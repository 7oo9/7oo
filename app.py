...
if len(scenic_df) < 10:
    ...
    ax.plot(scenic_df['ds'], scenic_df['y'], 'ko', markersize=4, label='历史观测值')
    ax.plot(forecast['ds'], forecast['yhat'], 'b-', linewidth=2, label='拟合曲线')
    ax.fill_between(forecast['ds'], forecast['yhat_lower'], forecast['yhat_upper'], color='blue', alpha=0.1, label='置信区间')
    
    max_date = scenic_df["ds"].max()
    pred_only = forecast[forecast["ds"] > max_date]
    ax.plot(pred_only["ds"], pred_only["yhat"], color="#d62728", linewidth=3, label="未来3个月预测")
    ax.fill_between(pred_only["ds"], pred_only["yhat_lower"], pred_only["yhat_upper"], color="#d62728", alpha=0.1)
    
    ax.set_xlabel("月度日期", fontsize=11)
    ax.set_ylabel("综合热度指数", fontsize=11)
    ax.set_title(f"{select_scenic} 热度时序拟合与短期预测", fontsize=13)
    ax.grid(alpha=0.25)
